...
describe('Game Logic', () => {
  ...
  describe('fetchGame', () => {
    it('should fetch game data successfully', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        'http://test-server.com/api/game/123',
        expect.any(Object)
      );
    });
  ...
  describe('checkGameState', () => {
    it('should check game state successfully', async () => {
      const mockResponse = { status: 'playing' };
      (global.fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(mockResponse),
      });

      const result = await checkGameState('player1', '123', ['', '', '', '', '', '', '', '', '']);
      expect(result).toEqual(mockResponse);
      expect(global.fetch).toHaveBeenCalledWith(
        'http://test-server.com/api/checkGameState',
        expect.any(Object)
      );
    });
  });
  ...
  describe('sendMove', () => {
    it('should send move successfully', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        'http://test-server.com/api/playerMove',
        expect.any(Object)
      );
    });
  ...
  describe('getAIMove', () => {
    it('should get AI move successfully', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(global.fetch).toHaveBeenCalledWith(
        'http://test-server.com/api/AIMove',
        expect.any(Object)
      );
    });
  });
  ...
  describe('getLeaderBoard', () => {
    it('should fetch leaderboard successfully', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        'http://test-server.com/api/leaderboard',
        expect.any(Object)
      );
    });
